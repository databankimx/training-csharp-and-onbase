...
using Samples.AsmxWebService.Models.Enumerations;
    ...
    public static class Database
    {
        ...
        public static List<Location> LookupLocations(this string zipCode)
        {
            try
            {
                ...
                zip = zipCode;
                if (DebugMode) Logger?.Debug("Lookup ZIP Code [{Zip}]...", zip);
            ...
            try
            {
                switch (Settings.Architecture)
                {
                    case DbArchitecture.Oracle:
                        if (DebugMode) Logger?.Debug("Connecting to Oracle database [{OracleTnsName}]...", Settings.OracleTnsName);
                    ...
                    case DbArchitecture.Odbc:
                        if (DebugMode) Logger?.Debug("Connecting to ODBC database [{OdbcDataSource}]...", Settings.OdbcDataSource);
        ...
        private static void ParameterizeQuery()
        {
            try
            {
                ...
                {
                    case DbArchitecture.Oracle:
                        #pragma warning disable S1192 // Lesson should keep literal
            ...
            try
            {
                ...
                if (DebugMode && locations.Count > 0)
                {
                    foreach (var location in locations)
                        Logger?.Debug("Found Location: [{LocationInfo}]", location.Info);
                }
