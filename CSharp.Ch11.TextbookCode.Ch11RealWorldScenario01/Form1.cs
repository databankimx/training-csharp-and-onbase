 ...
 * *Migration Note:
 *
 * One real bug was found and fixed here during the 2026 migration, in SanityCheckRow():
 * the quantity check re-tested "price" against minUnitCost/maxUnitCost (already checked
 * two lines above, for the SAME reason) instead of testing "quantity" against
 * minQuantity/maxQuantity, both of which are declared as constants at the top of this
 * file but were never actually referenced anywhere before this fix. This is a classic
 * copy-paste bug: the quantity check block was clearly copied from the unit price check
 * block immediately above it, and the variable names inside the condition were never
 * updated to match. Practical effect: quantity values were never sanity-checked at all
 * (an order for 100,000 units of something would never trigger the "unusual, continue
 * anyway?" prompt), while a merely-unusual price could incorrectly generate TWO messages
 * ("Unit price is unusual" and "Quantity is unusual") for what was really one issue.
 * See LectureNotes.md for further discussion.
 */
...
namespace Ch11RealWorldScenario01
{
    public partial class Form1 : Form
    {
        ...
        private void SanityCheckRow(int row, ref string message, ref TextBox focusTextBox)
        {
            ...
            // Check the quantity.
            // *Fixed*: this originally re-tested "price" against minUnitCost/maxUnitCost
            // (a copy-paste leftover from the block directly above), quantity was never
            // actually checked at all. Now correctly tests "quantity" against
            // minQuantity/maxQuantity, and focuses the actual quantity field, not the
            // unit cost field, when it fails.
            int quantity = int.Parse(rowTextBoxes[row, 2].Text);
            if ((quantity < minQuantity) || (quantity > maxQuantity))
            {
                message += "Quantity " + (row + 1) + " is unusual.\n";
                if (focusTextBox == null) focusTextBox = rowTextBoxes[row, 2];
            }
        }
